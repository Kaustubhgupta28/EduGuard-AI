...
def generate_viva_questions(doc_profile: dict, raw_text: str, gemini_api_key: str) -> list:
    """
...
def evaluate_answer(question: str, student_answer: str, expected_keywords: list,
                    marks: int, gemini_api_key: str) -> dict:
    """
    Evaluate a single student answer using LLM.
    Returns score, feedback, and missing concepts.
    """
    llm = ChatGoogleGenerativeAI(api_key=gemini_api_key, model="gemini-2.0-flash", temperature=0.2)
...
def run_verification_agent(state: dict) -> dict:
    ...
    questions = generate_viva_questions(doc_profile, raw_text, gemini_api_key)
    ...
    for i, q in enumerate(questions, 1):
        ...
        evaluation = evaluate_answer(question, student_answer, keywords, marks, gemini_api_key)
