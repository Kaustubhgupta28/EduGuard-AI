...
def compute_code_heuristic(text: str) -> dict:
    ...
    ai_likelihood += 10
    return {
        "heuristic_ai_score": round(max(0.0, min(100.0, ai_likelihood)), 1),
...
def llm_ai_analysis(text: str, content_type: str, gemini_api_key: str) -> dict:
    llm = ChatGoogleGenerativeAI(api_key=gemini_api_key, model="gemini-2.0-flash", temperature=0.0)
...
Return ONLY JSON. No markdown."""
    ...
    response = llm.invoke([
        ...
        HumanMessage(content=f"Analyze this {label} for AI-generation signals:\n\n{text[:5000]}")
    ])
...
def run_ai_detection_agent(state: dict) -> dict:
    ...
    semantic = llm_ai_analysis(raw_text, content_type, gemini_api_key)
    ...
    print(f"  Final AI Score   : {final_score}%  |  Risk: {risk}")
