...
def run_coach_agent(state: dict) -> dict:
    ...
    if not groq_api_key:
        return {
            **state,
            "study_roadmap": {
                "roadmap_title": "Roadmap unavailable",
                "total_days": 7,
                "daily_plan": [],
                "final_milestone": "Configure GROQ_API_KEY and rerun.",
                "motivational_note": "Keep going — you are close!",
            },
    ...
    areas_to_improve = feedback.get("student_feedback", {}).get(
        "areas_to_improve",
        [],
    )
...
Create a 7-day personalized study roadmap targeting the weak areas."""
    ...
    try:
        ...
        roadmap = {
            ...
            "daily_plan": [
                {
                    "day": i + 1,
                    "focus_topic": (
                        all_weak[i % len(all_weak)]
                        if all_weak
                        else f"Topic {i + 1}"
                    ),
                    "goal": (
                        f"Understand "
                        f"{all_weak[i % len(all_weak)]}"
                        if all_weak
                        else "core concepts"
                    ),
                    "tasks": [
            ...
            "final_milestone": (
                f"Strong conceptual understanding of {topic}"
            ),
            "motivational_note": (
    ...
    for day_plan in roadmap.get("daily_plan", []):
        ...
        resource_text = ", ".join(
            r.get("title", "") + " (" + r.get("platform", "") + ")"
            for r in resources
        )

        print(f"    Resources: {resource_text}")
